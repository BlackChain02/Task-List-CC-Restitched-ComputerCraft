...
local function drawButton(button, fillColor)
    ...
    -- Draw the label   
    monitor.setCursorPos(button.x + math.floor((buttonWidth - #button.label) / 2), button.y + math.floor(buttonHeight / 2))
...
local function drawAllButtons()
    ...
    local startY = math.floor((screenHeight - (2 * (buttonHeight + buttonSpacing) - buttonSpacing)) / 2)
    ...
    for i = 1, buttonCount do
        ...
        buttons[i] = {x = x, y = y, label = buttonLabels[i], index = i}
